...
### With [Apollo](https://www.apollographql.com) and Custom URL or Headers per Request

Note: This only works in distinct connection mode
...
class SSELink extends ApolloLink {
  ...
  // Feel free to adjust the options type here to be what you need to be dynamic
  constructor(options: ClientOptions) {
    super();
    this.client = createClient({
      ...options,
      // Optionally append the operation name to url
      url: (request) => {
        const url =
          typeof options.url === 'function' ? await options.url() : options.url;
        return url + `/${request.operationName}`;
      },
      // Merge headers from each operation's context
      headers: async (request) => {
        const operation: Operation = this.operationsByRequest.get(request)!;
        return {
          // This can be omitted if your created link's options don't include headers
          ...(typeof options.headers === 'function'
            ? await options.headers(request)
            : options.headers),
  ...
  public request(operation: Operation): Observable<FetchResult> {
    const request: RequestParams = {
      ...operation,
      // This supports the persisted queries link to avoid sending printed query when cached
      query:
        operation.getContext().http?.includeQuery ?? true
          ? print(op.query)
          : undefined!,
    };
